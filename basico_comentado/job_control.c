...
void get_command(char inputBuffer[], int size, char *args[],int *background)
{
	...
	*background=0;
	...
	for (i=0;i<length;i++)
	{
		//miramos la cadena que se nos pasa y separamos con delimitadores (como el split en java)
		...
		case '\t' :               /* argument separators */
			...
			//guardas en input buffer la cadena del comando con un  \0 al final para que sea cadena en c
			inputBuffer[i] = '\0'; /* add a null char; make a C string */
			start = -1;
			break;
				//en caso de \n, dado que es el ultimo caracter de la cadena se crea una cadena con un \0, es decir vacia
		...
		default :             /* some other character */
			//miramos si tiene el & de background, y si es asi modificacmos la variable bg
...
// -----------------------------------------------------------------------
//busca un elemento por la posicion que ocupa en la lista Viendolo ahora quizas me hubiera venido bien para el fg y bg
...
enum status analyze_status(int status, int *info)
{
	...
	{
		//devuelve el numero de señal que paro el proceso y ademas devuelve el proceso como suspendido
		*info=WSTOPSIG(status);
		return(SUSPENDED);
	}
	else
	{
		// el proceso termino por una señal
		if (WIFSIGNALED (status))
		{ *info=WTERMSIG (status); return(SIGNALED);}
		//si el proceso termina de forma natural devuelve terminado
		else if(WIFEXITED(status))
		{ *info=WEXITSTATUS(status); return(EXITED);}
			//si ha recibido sigcont, continua
		else{
			*info=0;
			return CONTINUED;
		}
...
// -----------------------------------------------------------------------
//bloquea una señal que se le pase como parametro;
void block_signal(int signal, int block)
{
	/* declara e inicializa máscara */
	sigset_t block_sigchld;
	//mira si la señal esta dentro de las que estan bloqueadas, y si no la añade al set de bloqueadas.
