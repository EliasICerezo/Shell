...
// void handler_SIGCHLD(job **l,int *idx){
// 	job *aux=*l;
//
// 	int status=0;
// 	int sign;
...
//
// 	while(aux!=NULL){
// 			//esto lo que nos devuelve es el pid de un proceso si ha terminado
//
...
//
// 					if(strcmp(status_strings[stat],"Suspended")==0 && aux->state!=BACKGROUND){
// 						printf("\nEstoy parando a %d\n",aux->pgid);
// 						aux->state=STOPPED;
//
//
